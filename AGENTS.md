...
- **Pre-commit hooks run via `nix flake check`, not as a standalone tool.** The flake's `checks.pre-commit-check` (generated by `pre-commit.nix`) is what enforces formatting/linting — there is no separate `pre-commit run` step. Run `nix flake check` to exercise it. Do not bypass it with `git commit --no-verify`.
