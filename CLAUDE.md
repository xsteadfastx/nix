...
- **Mechanism**: In `--extension` mode the server opens a CDP **relay** (`ws://…/extension/<uuid>`) and spawns a Chromium (via `--executable-path` + `PLAYWRIGHT_MCP_USER_DATA_DIR`, both pointing at your real profile) to open the extension's `connect.html`, which singleton-forwards into your **already-running** Chromium (Profile 1). The installed Playwright Extension then connects back to the relay and bridges the MCP server to your real, logged-in tab. Verified working via `pw:mcp:relay` debug lines: `CDP relay server started … Establishing extension connection … Extension connection established … Playwright MCP connected`.
- **⚠️ The `PLAYWRIGHT_MCP_ISOLATED` trap (0.0.69→0.0.76 regression, root-caused & fixed 2026-07-14)**: nixpkgs' `playwright-mcp` bin is a **wrapper** that runs `if [ -z "$PLAYWRIGHT_MCP_USER_DATA_DIR" ]; then export PLAYWRIGHT_MCP_ISOLATED=1; fi`. Since 0.0.76 (playwright-core **1.61**) the browser-mode decision chain evaluates `isolated` **before** `extension` (`remoteEndpoint → cdpEndpoint → isolated → extension → persistent`), so that forced `ISOLATED=1` silently **wins over `--extension`** and launches a throwaway temp-profile browser (`/tmp/playwright_chromiumdev_profile-* --disable-extensions`, no logins) — the "opens a new browser" symptom. 0.0.69 evaluated extension first, so it was latent. **Fix (updated 2026-09-25)**: set `PLAYWRIGHT_MCP_USER_DATA_DIR` in the server `env` to the **same real profile** as `PWTEST_EXTENSION_USER_DATA_DIR` (`/home/marv/.config/chromium`) so the wrapper skips the `ISOLATED=1` export. ⚠️ The earlier workaround used a tmpfs sentinel (`/run/user/<uid>/playwright-mcp-profile`) believing it inert — **since playwright-core 1.63 it is NOT inert**: `createExtensionBrowser` passes this value to `CDPRelayServer` as the connect-page browser's `--user-data-dir`, *overriding* `PWTEST_EXTENSION_USER_DATA_DIR`, so a sentinel launches a fresh, extension-less profile → "opens a new browser with an error". Because `--executable-path` is set, the `if (userDataDir && !executablePath && …) throw "Playwright Extension not found"` guard is skipped, so it fails silently. NB: `PLAYWRIGHT_MCP_ISOLATED=0` alone does **not** work — the wrapper re-forces it to `1` because it only checks `-z USER_DATA_DIR`. The relevant `--executable-path`/connect-page bug ([playwright#40557](https://github.com/microsoft/playwright/issues/40557)) was already fixed in 0.0.73, so that flag is honored — it was not the cause here.
- **MCP versions track `nixpkgs-unstable`** via `overlays/coding-agent.nix` (the `inherit (unstable) …` block), **except `pi-coding-agent` and `playwright-mcp`, which are version-pinned** in the coding-agent repo's own `overlay.nix`. `playwright-mcp` is pinned to `0.0.80` (with its coupled playwright-core `1.63.0`) via a dedicated `nixpkgs-playwright` flake input there — because its version is tightly coupled to the bundled playwright driver and a `1.63` change silently broke extension mode (upstream is already on `1.64`). The pin is guarded by the `coding-agent-playwright-pin` check. So a `flake update` here no longer moves playwright-mcp; to move it, bump the pin in the coding-agent repo and re-test extension mode.
- **Extension install**: The Playwright Extension (Web Store id `mmlmfjhmonkocbjadbfplnigmagldckm`, currently v0.4.0) must be installed **manually from the Chrome Web Store in the active profile** (here `Profile 1`, not `Default`). `home-manager/modules/chromium.nix` declares it, but HM's `programs.chromium.extensions` only seeds a **fresh** profile — on the pre-existing profile it never installed, so install it once by hand.
...
### i3 workspace-layout restore on dock (stale snapshot bug, fixed 2026-09-15)
- **Physical layout**: `eDP-1` (laptop, leftmost) + `DP-1-3` (middle external) + `DP-1-4` (right external). Desired i3 mapping on dock: workspace 1 → `DP-1-3` (middle), workspace 2 → `eDP-1` (left/laptop), workspace 3 → `DP-1-4` (right) — this is the default fallback branch already coded in `hosts/coltrane/autorandr.nix`'s `move-workspaces` postswitch hook.
- **The bug**: `hosts/coltrane/lock.nix`'s `lock` script snapshots the current i3 workspace→output layout into `/run/user/1000/autorandr-ws-layout` on **every** lock (manual idle-lock included, not just suspend). If that lock fires while undocked (single output), i3 reports every workspace's output as `eDP-1` — a degenerate snapshot. `move-workspaces` always prefers a present snapshot file over its sane MIDDLE/eDP-1/RIGHT default, so the next dock event replayed the junk snapshot and dumped every workspace onto the laptop screen instead of spreading across all three monitors. Confirmed live via `journalctl -u autorandr`: a good docked snapshot (`ws1→DP-1-3, ws3→DP-1-4`) got clobbered by an all-`eDP-1` one from an intervening mobile-mode lock.
- **Fix applied**: `lock.nix` now only writes `autorandr-ws-layout`/`autorandr-visible-ws` when `xrandr --query` shows more than one connected output (i.e. actually docked); `autorandr-current-ws` (focus restore) is still always captured. Committed (158d390), built, and switched 2026-09-15 — confirmed live (xss-lock auto-restarted onto the new `lock` script on activation).
- **Separately**: the MST/monitor-drop-after-sleep saga ([[project_coltrane_mst_cleanup]] in agent memory) currently looks resolved in practice — a live check on 2026-09-15 showed all three outputs correctly enumerated and positioned after a normal resume, with `xe.enable_dc=0` + `xe.enable_psr=0` active on the kernel cmdline. **New empirical finding (user, 2026-09-16): monitors reliably come back if both external monitors are already powered on when the laptop wakes** — waking with the monitors themselves asleep/off is the case that still loses MST. Consistent with the resume-time HPD/link-relink theory: the sink needs to be powered to assert hotplug for `xe` to see it. Practical workaround: power on external monitors before waking the laptop. Not yet re-verified after a long overnight/weekend sleep with monitors powered throughout.

### Web Serial "device has been lost" (MeshCore / CP210x, root-caused 2026-09-24)
- **Symptom**: port shows in Chromium's picker, connecting fails. Two distinct causes:
- **Cause 1 — baud rate, glibc ≥ 2.42** (MeshCore app: "Failed to fetch device info … timeout"): glibc 2.42 made `B115200` equal the plain number `115200` (`0o341000`, was `0o10002`). Chromium's `ConfigurePortImpl` still does `c_cflag &= ~CBAUD; c_cflag |= B115200` and pushes it raw via `TCSETS2`, so `CBAUD` ends up `B0` and the chip **keeps its previous speed** (9600 after a fresh plug). Tell-tale after a failed session: `stty -a` shows `ispeed 9600 baud; ospeed 0 baud`. Same bug class as BlueZ `btattach` (patched Sept 2026). Explains "worked a year ago" and why it "sometimes worked" (whenever something had already set 115200). Workaround: udev pre-sets 115200; a flasher that changes baud mid-flash still breaks. Real fix: patch Chromium to use `BOTHER` + `c_ospeed` (upstream report pending).
- **Cause 2 — VMIN** (console: `Error reading from serial port: NetworkError: The device has been lost.`): the tty has `VMIN=0 VTIME=0`. Chromium's `ConfigurePortImpl` never sets `VMIN`/`VTIME` (it inherits them), and with `VMIN=0` a non-blocking `read()` returns `0` instead of `EAGAIN` — Chromium maps `0` to `DEVICE_LOST`. **pyserial** (esptool, meshcore-cli, pio monitor) always leaves `VMIN=0`, and the kernel keeps termios per tty index across close and unplug/replug until reboot. `stty sane` does not reset min/time (the earlier fix attempts never touched it). Hardware/driver/permissions were fine: a raw Python probe got a `CMD_DEVICE_QUERY` reply.
- **Fix**: the udev rule in `hosts/coltrane/configuration.nix` runs `stty 115200 min 1 time 0` on plug-in (covers both causes). After using a pyserial tool, re-plug (or run `stty -F /dev/ttyUSB0 115200 min 1 time 0`) before using Web Serial.
- **Debugging it**: `stty -F /dev/ttyUSB0 -a | grep min`; for Chromium's side, launch a throwaway `chromium --user-data-dir=$(mktemp -d) --enable-logging=stderr` and grep for `CONSOLE`.

### Waybar self-healing (2026-09-24)
- Waybar never recovers on its own: HM's default SIGUSR2 **reload** on switch leaks state (duplicate tray hosts); the unpinned `network` module sticks on "offline" after the default route bounces (resume/dock) with ppp0/tailscale0 up -- now split into `network#wifi` (`wlp*`) + `network#nic` (`enp*`, hidden when down), and the `wireplumber` module never reconnects after the audio stack restarts (0% forever). `home-manager/modules/waybar/default.nix` now forces **restart** triggers instead of reload, and `BindsTo=wireplumber.service` + `sway-session.target` `Upholds=waybar.service` so an audio restart/kill bounces the bar automatically.
- Gotcha that triggered it: `pkill pi` (pattern match) also kills `pipewire`/`pipewire-pulse`; use `pkill -x pi`. A Bluetooth sink that connects while wireplumber is down stays "connected" with no A2DP ("Protocol not available") until reconnected.
...
- `playwright`: Drives already-open Chromium tabs via the Playwright browser extension (extension mode). Requires `--executable-path` + `PWTEST_EXTENSION_USER_DATA_DIR` **and** `PLAYWRIGHT_MCP_USER_DATA_DIR`, **both set to the real profile** (`/home/marv/.config/chromium`) — the latter defeats the nixpkgs wrapper's forced `PLAYWRIGHT_MCP_ISOLATED=1` **and** is used as the connect-page browser's user-data-dir, so a throwaway value opens an extension-less browser (see the **Playwright Extension Mode** section).
- `memory`: Persistent knowledge-graph memory at `~/.pi/agent/memory.jsonl`.
- `postgres-hemingway-barletta`: Read-only access to **barletta's `hemingway` DB** via `crystaldba/postgres-mcp` over a lifecycle-bound SSH **Unix-socket** forward (see **Postgres MCP** below). Keyed by tech+DB+host (the key prefixes every tool name), mirroring the two `grafana-*` instances.

### Postgres MCP (barletta, read-only)
- **Package**: `crystaldba/postgres-mcp` ("Postgres MCP Pro", ~3k★) — the popular, maintained one, **not** the archived MCP-monorepo server (whose "read-only" ran `DROP SCHEMA`). Not in nixpkgs, so built from source in `pkgs/postgres-mcp.nix` and wired into `overlays/coding-agent.nix` via `callPackage`. Every runtime dep is already in nixpkgs-unstable, so plain `buildPythonApplication` resolves it — no uv2nix/lockfile needed; only `pglast`'s `==7.11` pin is relaxed (nixpkgs has 7.13).
- **Transport to barletta**: barletta's postgres isn't exposed on the network. A `writeShellApplication` wrapper (`postgresHemingwayBarletta` in `hosts/coltrane/coding-agent.nix`) forwards barletta's postgres **Unix socket** (`/run/postgresql/.s.PGSQL.5432`) into a per-instance `mktemp -d` dir **bound to the MCP's lifetime** (`ssh -f -N -M` control master; EXIT trap runs `ssh -O exit` and `rm -rf`s the socket dir), then runs `postgres-mcp --access-mode restricted` against `host=$SOCKDIR` in the **foreground**. It's spawned by pi/Claude so it inherits the agent's SSH keychain env (a `systemd.user` unit would not see `SSH_AUTH_SOCK`).
- **⚠️ One socket dir per instance (concurrency)**: pi and Claude **share** `~/.pi/agent/mcp.json`, so both spawn this wrapper simultaneously. An earlier design forwarded a **fixed TCP port (5433)** — only one client can bind it; the second's `ssh -L` hit `ExitOnForwardFailure`, `set -e` aborted the wrapper, `postgres-mcp` never started, and that client died with **`-32000` Connection closed**. Fix: a fresh `mktemp -d` socket dir per launch, so instances never collide (each `EXIT` trap tears down only its own tunnel — deliberately **not** a shared/reference-counted tunnel, which would couple lifecycles). Verified: two concurrent `initialize` handshakes both succeed.
- **⚠️ The stdin `-32000` gotcha**: the MCP server MUST run in the **foreground**, not backgrounded (`&`). A backgrounded job in a non-interactive shell has stdin redirected to `/dev/null` (POSIX async-list rule), so the server never receives the client's `initialize` over stdio and the client fails with JSON-RPC `-32000`. It is deliberately not `exec`'d so the EXIT trap still tears the tunnel down and removes the socket dir.
- **No role / password / sops secret**: barletta's `pg_hba` has `local all all trust`, so connecting over the forwarded Unix socket needs no password — the connection string (`postgresql://postgres@/hemingway?host=$SOCKDIR`) is non-secret and inlined. The Unix socket also sidesteps the old TCP IPv4/IPv6 gotcha (barletta resolved `localhost` → `::1`, missing the IPv4 trust rule).
- **`postgres` superuser, not `readonly`**: the `readonly` role has **no data grants** on this instance and can't be fixed here — barletta's hemingway DB is a **read-only replica** (secondary of gerwer, `pg_is_in_recovery()=t`), so `GRANT`s can't run on it (they'd have to happen on the primary). The socket's `local all all trust` grants the whole cluster as any role, so the wrapper takes `postgres` (superuser) for full read access. DB `hemingway` is required in the DSN: it holds the device data (`public.recordings`) and postgres has no cross-DB queries — the `postgres` maintenance DB can't see it.
- **⚠️ Replica read-conflict**: long/full-scan reads on the standby may fail with `canceling statement due to conflict with recovery` (a WAL-replay conflict, tuned by barletta's `max_standby_streaming_delay` / `hot_standby_feedback` — a knob in the **infrastructure** repo, not here). Affects any read client on the replica; prefer bounded queries (`LIMIT`, indexed lookups).
- **Read-only** is enforced twice: the physical read-only replica **and** `--access-mode restricted` (pglast SQL-parse rejection).
- **No mcp-registry entry**: the host supplies `bin`/`command` directly (the tunnel wrapper is host-specific glue, not a generic package); `resolveServer` still runs it through `mkSecretWrapper` (a no-op here since there's no `*_FILE` env).
