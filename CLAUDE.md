...
- **Fix applied**: `lock.nix` now only writes `autorandr-ws-layout`/`autorandr-visible-ws` when `xrandr --query` shows more than one connected output (i.e. actually docked); `autorandr-current-ws` (focus restore) is still always captured. Built + closure-verified; not yet switched.
- **Separately**: the MST/monitor-drop-after-sleep saga ([[project_coltrane_mst_cleanup]] in agent memory) currently looks resolved in practice — a live check on 2026-09-15 showed all three outputs correctly enumerated and positioned after a normal resume, with `xe.enable_dc=0` + `xe.enable_psr=0` active on the kernel cmdline. Re-verify after any long (overnight/weekend) s2idle before declaring it fully fixed.
